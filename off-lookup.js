...
(function(){
  ...
  function nutrition(product){
    ...
    return {calories,protein:number(n.proteins_100g),carbs:number(n.carbohydrates_100g),fat:number(n.fat_100g)};
  }
  ...
  function openManualWithPrefill(code,product=null,values=null){
    ...
    setTimeout(()=>{
      if(product){
        ...
        if(values){
          [['#libCalories',values.calories],['#libProtein',values.protein],['#libCarbs',values.carbs],['#libFat',values.fat]].forEach(([selector,value])=>{const input=$(selector);if(input&&value>0)input.value=value;});
        }
      }
