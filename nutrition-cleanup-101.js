...
(function(){
  ...
  function removeRedundantText(){root?.querySelectorAll('.nutrition-coach-row').forEach(node=>node.remove());const text=$('#nutritionV7AnalysisText',root);if(text){text.textContent='';text.hidden=true}}
  ...
  function syncState(analysis,head){const collapsed=analysis.classList.contains('is-collapsed');head.setAttribute('aria-expanded',String(!collapsed));head.setAttribute('aria-label',collapsed?'Zusatzwerte anzeigen':'Zusatzwerte ausblenden')}
