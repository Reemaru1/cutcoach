...
(function(){
  ...
  function sync(){scheduled=false;ensureRoot();if(!root?.isConnected||typeof document==='undefined'||!document.body?.classList.contains('nutrition-mode'))return;syncCount++;removeRedundantText();normalizeMacroLabels();enhanceAnalysis()}
  function queue(){if(scheduled||typeof requestAnimationFrame!=='function')return;scheduled=true;requestAnimationFrame(sync)}
  ...
  function ensureRoot(){if(typeof document==='undefined'||typeof document.querySelector!=='function')return null;const found=document.querySelector('[data-screen="food"]');if(found&&found!==root)start(found);else if(!found&&root&&!root.isConnected){observer?.disconnect();observer=null;root=null}return root}
  function boot(){if(typeof document==='undefined')return;ensureRoot();if(!bootstrapObserver){bootstrapObserver=new MutationObserver(()=>{ensureRoot();queue()});bootstrapObserver.observe(document.body||document.documentElement,{childList:true,subtree:true})}queue()}
