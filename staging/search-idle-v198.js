'use strict';
(function(){
  const VERSION='1.9.8-staging';
  const IDLE_MS=230;
  const PASTE_MS=110;
  const MIN_QUERY_LENGTH=2;
  let timer=0,token=0,pendingInput=null,nativeInputCount=0,releaseCount=0,lastReason='',lastDelay=IDLE_MS,composing=false;

  const isSearchInput=input=>input?.id==='nutritionSearch';
  ...
  function release(input,reason='idle'){
    if(!isSearchInput(input)||!input.isConnected)return false;
    ...
    try{window.dispatchEvent(new CustomEvent('cutcoach:staging-search-release',{detail:{value:input.value,reason,delay:lastDelay}}))}catch{}
    return true;
  }

  function schedule(input,event){
    clearTimeout(timer);timer=0;pendingInput=input;const current=++token;nativeInputCount++;
  ...
  function handleBeforeInput(event){
    ...
    if(!isSearchInput(input)||bypassed(input)||composing)return;
  ...
  function handleKeydown(event){
    const input=event.target;if(!isSearchInput(input))return;
  ...
  function handleCompositionStart(event){
    if(!isSearchInput(event.target))return;composing=true;clearPending();
  }

  function handleCompositionEnd(event){
    const input=event.target;if(!isSearchInput(input))return;composing=false;queueMicrotask(()=>{if(input.isConnected)schedule(input,event)});
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    document.documentElement.dataset.searchIdleStaging='1';
    const badge=document.querySelector('.cc-staging-badge');if(badge)badge.textContent='STAGING · SUCHE 230 MS';
  },{once:true});

  window.CutCoachStagingSearchIdle198=Object.freeze({
    version:VERSION,
    idleMs:IDLE_MS,
    pasteMs:PASTE_MS,
    ...
    cancel:clearPending,
    stats:()=>Object.freeze({pending:Boolean(timer),nativeInputCount,releaseCount,lastReason,lastDelay})
  });
})();
