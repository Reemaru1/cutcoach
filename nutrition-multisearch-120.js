...
(function(){
  ...
  function ensureFirstHardenedRender(){
    if(document.querySelector('#nutritionMultiSearch')){queueMicrotask(refreshCurrent);return}
    if(hostBootstrap)return;
    hostBootstrap=new MutationObserver(()=>{if(!document.querySelector('#nutritionMultiSearch'))return;hostBootstrap.disconnect();hostBootstrap=null;queueMicrotask(refreshCurrent)});
    hostBootstrap.observe(document.body||document.documentElement,{childList:true,subtree:true});
  }
