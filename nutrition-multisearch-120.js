...
(function(){
  ...
  const ASSETS=[
    ...
    {selector:'script[data-nutrition-search-exact-whole-v170],script[data-search-exact-whole-v170]',dataset:'nutritionSearchExactWholeV170',src:'./nutrition-search-exact-whole-v170.js?v=1.7.1-alpha',ready:()=>Boolean(window.CutCoachSearchExactWhole170)},
    {selector:'script[data-nutrition-search-confidence-hardening-v151],script[data-confidence-hardening-v151]',dataset:'nutritionSearchConfidenceHardeningV151',src:'./nutrition-search-confidence-hardening-v151.js?v=1.6.1-alpha',ready:()=>Boolean(window.CutCoachSearchConfidenceHardening151)}
  ];
  ...
  function refreshCurrent(){const current=engine(),field=input();if(!current||!field)return false;if(String(field.value||'').trim())current.render?.(field);return true}
  ...
  function loadAsset(index){if(index>=ASSETS.length){attachLayers();return}const asset=ASSETS[index];if(asset.ready()){loadAsset(index+1);return}const page=currentDocument();if(!page)return;let completed=false;const next=()=>{if(completed)return;completed=true;loadAsset(index+1)};let script=page.querySelector?.(asset.selector);if(script){const previousLoad=script.onload,previousError=script.onerror;script.onload=event=>{previousLoad?.call(script,event);next()};script.onerror=event=>{previousError?.call(script,event);next()};queueMicrotask(()=>{if(asset.ready())next()});return}script=page.createElement('script');script.src=asset.src;script.async=false;script.dataset[asset.dataset]='1';script.onload=next;script.onerror=next;(page.head||page.documentElement)?.append(script)}
