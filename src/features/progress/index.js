...
(function(root){
  ...
  function loadBodySystem(){
    if(root.CutCoachBodySystem302)return Promise.resolve(root.CutCoachBodySystem302);
    if(bodyLoadPromise)return bodyLoadPromise;
    bodyLoadPromise=new Promise(resolve=>{
      try{
        const style=ensureAsset({id:'progress-body-v302-style',type:'style',href:'./src/features/progress/body-system-v302.css?v=3.0.2-alpha'});
        const script=ensureAsset({id:'progress-body-v302-script',type:'script',href:'./src/features/progress/body-system-v302.js?v=3.0.2-alpha'});
        const finish=()=>resolve(root.CutCoachBodySystem302||null);
        if(root.CutCoachBodySystem302){finish();return;}
  ...
  root.CutCoachModules?.register({
    ...
    onEnter:()=>{
      ...
      loadBodySystem().then(system=>{try{setTimeout(()=>system?.render?.(),60)}catch(error){console.warn('Body System Renderfehler:',error)}});
