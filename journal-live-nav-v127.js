...
(function(){
  ...
  let renderWrapped=false,libraryWrapped=false,observer=null;
  ...
  function restoreNav(){
    ...
    for(const button of nav.querySelectorAll('[data-tab]')){const key=button.dataset.tab,label=navLabels[key];if(!label)continue;if(button.dataset.nav127==='1'||button.querySelector('.nav-v127-icon,.nav-v127-label'))button.innerHTML=`<span aria-hidden="true">${navIcons[key]}</span>${label}`;delete button.dataset.nav127;const active=button.classList.contains('active')||button.getAttribute('aria-current')==='page';button.setAttribute('aria-label',`${label}${active?', ausgewählt':''}`)}
  }
  function runRefresh(){try{window.CutCoachJournalV72?.render?.()}catch{}try{window.CutCoachJournalV740?.refresh?.()}catch{}try{window.CutCoachJournalV71?.render?.()}catch{}fixStepCopy();decorateActions();restoreNav()}
  function clearRefreshTimers(){for(const timer of refreshTimers)clearTimeout(timer);refreshTimers.clear()}
  function scheduleRefresh(delay){const timer=setTimeout(()=>{refreshTimers.delete(timer);runRefresh()},delay);refreshTimers.add(timer)}
  function refreshJournal(){clearRefreshTimers();scheduleRefresh(0);scheduleRefresh(100);scheduleRefresh(260)}
  ...
  function decorateActions(){const root=$('[data-screen="food"]')||document;for(const button of root.querySelectorAll('button')){const text=(button.textContent||'').trim();if(text==='Manuell'){if(button.lastChild)button.lastChild.textContent='Schnell anlegen';button.setAttribute('aria-label','Lebensmittel mit eigenen Nährwerten schnell anlegen')}else if(text==='Lebensmittel'){if(button.lastChild)button.lastChild.textContent='Suchen';button.setAttribute('aria-label','Einzelnes Lebensmittel in der Datenbank suchen')}else if(text==='Rezept')button.setAttribute('aria-label','Rezept aus mehreren Zutaten erstellen oder auswählen');else if(text==='Barcode')button.setAttribute('aria-label','Produkt über den Barcode suchen')}}
  function installObserver(){if(observer)return;observer=new MutationObserver(records=>{if(records.some(record=>record.addedNodes.length||record.removedNodes.length)){wrapRender();wrapLibrary();decorateActions();restoreNav();fixStepCopy()}});observer.observe(document.body,{childList:true,subtree:true})}
  function start(){wrapRender();wrapMutationFunction('commitDayMutation');wrapMutationFunction('commitStateMutation');wrapLibrary();decorateActions();restoreNav();fixStepCopy();installObserver();refreshJournal();document.addEventListener('cutcoach:data-changed',refreshJournal);document.addEventListener('click',event=>{if(event.target.closest?.('#saveMeal,[data-nutrition-add],[data-canonical-add],[data-canonical-all],[data-delete-meal],[data-edit-meal],#copyPreviousMeals'))refreshJournal()},true);window.addEventListener('pageshow',refreshJournal);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshJournal()})}
