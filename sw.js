...
const CACHE_NAME='cutcoach-v3.3.0';
...
const APP_SHELL=[
  ...
  './library-init.js?v=3.0.0','./library.css?v=3.0.0','./scanner-v2.js?v=3.1.0','./off-lookup.js?v=3.3.0','./update.html'
];
...
function injectApp(html){
  ...
  if(!page.includes('off-lookup.js?v=3.3.0'))scripts.push('<script src="off-lookup.js?v=3.3.0" defer></script>');
...
self.addEventListener('fetch',event=>{
  ...
  if(request.mode==='navigate'){
    event.respondWith((async()=>{try{const network=await fetch(request,{cache:'no-store'});const page=await preparePage(network);if(page.ok){const cache=await caches.open(CACHE_NAME);await cache.put('./index.html',page.clone());}return page;}catch{return (await caches.match('./index.html'))||Response.error();}})());
    return;
  }
