...
const CACHE_NAME='cutcoach-v3.0.0';
const APP_SHELL=[
  ...
  './apple-touch-icon.png?v=2.3.0','./icon-192.png?v=2.3.0','./library.js?v=3.0.0','./library-init.js?v=3.0.0','./library.css?v=3.0.0'
];

function injectLibrary(html){
  if(html.includes('library.js?v=3.0.0'))return html;
  return html.replace('</body>','<script src="library.js?v=3.0.0" defer></script><script src="library-init.js?v=3.0.0" defer></script></body>');
}
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
    ...
    try{const response=await fetch(request,{cache:'no-store'});if(response.ok){const cache=await caches.open(CACHE_NAME);await cache.put(request,response.clone());}return response;}catch{return Response.error();}
  })());
});
