...
const UI_CACHE=`${CATALOG_CACHE}-search192-ui-overhaul`;
...
const TYPING_CACHE=`${ARTICLE_CACHE}-search197-typing-performance`;
const IDLE_CACHE=`${TYPING_CACHE}-search198-idle-live`;
const KEYBOARD_CACHE=`${IDLE_CACHE}-search199-ios-keyboard`;
...
const VOICE_CACHE=`${INTENT_CACHE}-voice203-direct-permission-nutrition204-runtime-hardening-nutrition205-a-z-hardening-nutrition206-math-ui-ui207-liquid-glass-ui208-production-loader-ui209-hidden-scrollbars-body214-rollback-body221-production-audit-nutrition230-profile920-profile1000-profile1002-loader-profile1004-separation-progress230-profile1005-auto-sync-profile1006-layout-profile1007-spacing-profile1008-bottom-clearance-profile1009-settings-polish-profile1010-delivery-profile1011-settings-icon-profile1012-flat-header-profile1013-forced-delivery-profile1016-settings-emergency-rollback-profile1020-onboarding-one-time-body300-emergency-rollback-body301-safe-lazy-body302-progress-first-pass-body240-progress-refinement-profile1021-editor-wizard-nutrition250-online-products-catalog251-local-products-catalog260-retailer-expansion-search261-runtime-catalog-catalog270-kinder-products-bootstrap271-search272-live-catalog-nutrition273-day-progress-nutrition274-init-fix-nutrition275-direct-render-nutrition276-visual-polish-nutrition277-live-loader`;
...
async function refreshNavigation(request){const network=await fetchWithTimeout(request,{cache:'no-store'},NAVIGATION_TIMEOUT_MS),page=await preparePage(network);if(page.ok)await(await caches.open(CACHE_NAME)).put('./index.html',page.clone());return page}
...
async function precacheFresh(cache,asset){const request=new Request(asset,{cache:'reload'}),response=await fetch(request);if(!response.ok&&response.type!=='opaque')throw new Error(`Precache failed: ${asset}`);await cache.put(asset,response)}
self.addEventListener('install',event=>event.waitUntil((async()=>{const cache=await caches.open(CACHE_NAME);await Promise.all([...new Set(APP_SHELL)].map(asset=>precacheFresh(cache,asset)));await Promise.allSettled(EXTERNAL_ASSETS.map(asset=>cache.add(asset)));await self.skipWaiting()})()));
