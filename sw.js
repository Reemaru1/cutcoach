...
const CACHE_NAME=`cutcoach-v${RUNTIME.version}-nav132`;
...
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE_NAME).then(cache=>cache.addAll([...new Set(APP_SHELL)])).then(()=>self.skipWaiting())));
