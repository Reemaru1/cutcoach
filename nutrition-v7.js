...
(function(){
  ...
  function renderAnalysis(){
    ensureAnalysis();const root=$('#nutritionV7Analysis');if(!root||typeof totals!=='function'||typeof day!=='function')return;const total=totals(selectedDate),meals=day(selectedDate,false).meals||[],count=Math.max(1,meals.length),coverage=total.nutrientCoverage||{},ratios=['fiber','sugar','saturatedFat','salt'].map(key=>Math.min(1,(Number(coverage[key])||0)/count)),average=meals.length?ratios.reduce((a,b)=>a+b,0)/ratios.length:0;
    $('#nutritionV7Coverage').textContent=`${Math.round(average*100)} %`;$('#nutritionV7Fiber').textContent=meals.length&&coverage.fiber?`${fmt(total.fiber,1)} g`:'–';$('#nutritionV7Sugar').textContent=meals.length&&coverage.sugar?`${fmt(total.sugar,1)} g`:'–';$('#nutritionV7Sat').textContent=meals.length&&coverage.saturatedFat?`${fmt(total.saturatedFat,1)} g`:'–';$('#nutritionV7Salt').textContent=meals.length&&coverage.salt?`${fmt(total.salt,2)} g`:'–';$('#nutritionV7AnalysisText').textContent=!meals.length?'Zusatzwerte erscheinen, sobald Mahlzeiten eingetragen sind.':average<.5?'Die Analyse ist noch unvollständig, weil mehrere Lebensmittel keine Zusatzwerte liefern.':average<.9?'Ein Teil der Zusatzwerte fehlt. Angezeigte Summen deshalb vorsichtig interpretieren.':'Die Zusatzwerte sind für fast alle heutigen Einträge vorhanden.';
  }
  ...
  function captureClick(event){
    ...
    const add=event.target.closest?.('[data-nutrition-add]');if(add&&document.body.classList.contains('nutrition-mode')){const key=`${selectedDate}|${mealType()}|${add.dataset.nutritionAdd}`,now=Date.now();if(lastAdd.key===key&&now-lastAdd.at<ADD_LOCK_MS){event.preventDefault();event.stopImmediatePropagation();toast?.('Dieser Eintrag wurde bereits hinzugefügt.');return}lastAdd={key,at:now};add.setAttribute('aria-busy','true');setTimeout(()=>add.removeAttribute('aria-busy'),ADD_LOCK_MS)}
