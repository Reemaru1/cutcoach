...
(function(){
  ...
  function clearStaleResults(){try{window.CutCoachNutritionStage6?.clear?.('voice-preview')}catch{}const host=$('#nutritionMultiSearch');if(host){host.hidden=true;host._canonicalRows=null;delete host.dataset.canonical;delete host.dataset.query}}
  ...
  function restoreInitial(){const {input}=nodes();if(!input||(!sessionActive&&input.dataset.voicePreview!=='1'))return false;input.value=initialValue;delete input.dataset.voicePreview;return true}
  ...
  function stop(commit=true){
    ...
    if(!instance){if(!commit&&sessionActive)restoreInitial();cleanup();return false}
    clearTimer();commitOnEnd=Boolean(commit);
    if(!commit){session++;restoreInitial();recognition=null;try{instance.abort()}catch{}cleanup();return true}
    setButton('processing');
    try{instance.stop();return true}catch{
      const result=normalize(finalText||interimText);
      if(result){cleanup();dispatchSearch(result)}else{restoreInitial();cleanup()}
      return false;
    }
  }
  ...
  function prepareButton(){const button=$('#nutritionVoice');if(!button||button.dataset.voice190)return false;button.dataset.voice190='1';button.setAttribute('aria-pressed','false');button.setAttribute('aria-busy','false');button.title='Lebensmittel per Sprache suchen';return true}
  ...
  window.CutCoachNutritionVoice111=Object.freeze({version:VERSION,start:begin,stop,state:()=>state,maxTranscript:MAX_TRANSCRIPT,sessionActive:()=>sessionActive});
})();
