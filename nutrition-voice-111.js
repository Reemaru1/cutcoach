...
(function(){
  ...
  function clearStaleResults(){
    try{window.dispatchEvent(new CustomEvent('cutcoach:search-input-pending',{detail:{source:'voice-preview'}}))}catch{}
    try{window.CutCoachNutritionStage6?.clear?.('voice-preview')}catch{}
    const host=$('#nutritionMultiSearch');if(host){host.hidden=true;host.replaceChildren();host._canonicalRows=null;host._v192Rows=null;delete host.dataset.canonical;delete host.dataset.presentationV192;delete host.dataset.query;delete host.dataset.renderSignature}
    restoreSuppressed();
  }
  ...
  function stop(commit=true){
    ...
    try{instance.stop();return true}catch{const result=normalize(finalText||interimText);if(result){cleanup();dispatchSearch(result)}else{restoreInitial();cleanup()}return false}
  }
  ...
  function prepareButton(){const button=$('#nutritionVoice');if(!button||button.dataset.voice191)return false;button.dataset.voice191='1';button.setAttribute('aria-pressed','false');button.setAttribute('aria-busy','false');button.title='Lebensmittel per Sprache suchen';return true}
