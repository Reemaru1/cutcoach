...
(function(){
  ...
  const PERMISSION_GRANTED_KEY='cutcoach_voice_permission_granted_v1';
  if(window.CutCoachNutritionVoice111)return;
  let recognition=null,state='idle',finalText='',interimText='',initialValue='',startedAt=0,timeoutId=0,session=0,commitOnEnd=true,sessionActive=false,permissionCheckToken=0;
  const $=selector=>document.querySelector(selector);
  const normalize=value=>String(value||'').replace(/\s+/g,' ').trim().slice(0,MAX_TRANSCRIPT);
  const isIOS=()=>/(?:iPhone|iPad|iPod)/i.test(navigator.userAgent||'')||navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1;
  const wasGranted=()=>{try{return localStorage.getItem(PERMISSION_GRANTED_KEY)==='1'}catch{return false}};
  const rememberGranted=()=>{try{localStorage.setItem(PERMISSION_GRANTED_KEY,'1')}catch{}};
  async function microphonePermission(){if(!navigator.permissions?.query)return'unknown';try{return(await navigator.permissions.query({name:'microphone'})).state||'unknown'}catch{return'unknown'}}
  ...
  function setButton(next){const {button}=nodes();state=next;if(!button)return;button.classList.toggle('listening',next==='listening');button.classList.toggle('processing',next==='processing');button.setAttribute('aria-pressed',String(next==='listening'));button.setAttribute('aria-busy',String(next==='processing'));button.setAttribute('aria-label',next==='listening'?'Spracheingabe beenden':next==='processing'?'Sprache wird vorbereitet':'Spracheingabe starten');button.title=next==='listening'?'Aufnahme beenden':'Lebensmittel per Sprache suchen'}
  ...
  function focusForDictation(message){cleanup();const {input}=nodes();setStatus(message,'fallback');input?.focus({preventScroll:true});try{input?.setSelectionRange(input.value.length,input.value.length)}catch{}}
  function fallback(message='Nutze die Diktierfunktion der iPhone-Tastatur.'){focusForDictation(message)}
  function persistentPermissionHint(){focusForDictation('Safari steht für diese Seite noch auf „Fragen“. Öffne CutCoach einmal in Safari und stelle unter Seitenmenü → Mehr → Website-Einstellungen → Mikrofon auf „Erlauben“. Bis dahin kannst du das Mikrofon der iPhone-Tastatur nutzen.')}
  function stop(commit=true){const instance=recognition;permissionCheckToken++;if(!instance){if(!commit&&sessionActive)restoreInitial();cleanup();return false}clearTimer();commitOnEnd=Boolean(commit);if(!commit){session++;restoreInitial();recognition=null;try{instance.abort()}catch{}cleanup();return true}setButton('processing');try{instance.stop();return true}catch{const result=normalize(finalText||interimText);if(result){cleanup();dispatchSearch(result)}else{restoreInitial();cleanup()}return false}}
  function errorMessage(code){return code==='not-allowed'||code==='service-not-allowed'?'Mikrofonzugriff blockiert. Stelle in Safari für diese Website das Mikrofon auf „Erlauben“ oder nutze die iPhone-Diktierfunktion.':code==='audio-capture'?'Kein Mikrofon verfügbar. Prüfe, ob eine andere App das Mikrofon verwendet.':code==='network'?'Spracherkennung benötigt gerade eine stabile Internetverbindung.':code==='no-speech'?'Keine Sprache erkannt. Tippe erneut und sprich direkt nach dem Signal.':code==='aborted'?'Spracheingabe beendet.':'Spracheingabe konnte nicht abgeschlossen werden.'}
  function startRecognition(){
    ...
    instance.onstart=()=>{if(token!==session)return;rememberGranted();setButton('listening');setStatus('Sprich jetzt – zum Beenden erneut tippen.','listening');timeoutId=setTimeout(()=>{if(token===session&&state==='listening')stop(true)},12000)};
  ...
  function begin(){
    ...
    const check=++permissionCheckToken;
    if(!isIOS()||!wasGranted()||!navigator.permissions?.query)return startRecognition();
    setButton('processing');setStatus('Mikrofonfreigabe wird geprüft …','info');
    microphonePermission().then(permission=>{if(check!==permissionCheckToken)return;if(permission==='granted')startRecognition();else if(permission==='denied')fallback('Mikrofon ist für diese Website blockiert. Stelle es in Safari auf „Erlauben“ oder nutze die iPhone-Diktierfunktion.');else persistentPermissionHint()});
    return true;
  }
  ...
  window.CutCoachNutritionVoice111=Object.freeze({version:VERSION,start:begin,stop,state:()=>state,maxTranscript:MAX_TRANSCRIPT,sessionActive:()=>sessionActive,clearStaleResults,microphonePermission,wasGranted,isIOS});
})();
