...
(function(){
  ...
  function priorityModel(total,data,settings,water){
    ...
    if(water<Math.max(250,pace-250))add(94,'water','Trinkplan aufholen',`${fmt(Math.max(250,pace-water))} ml fehlen bis zum zeitabhängigen Trinkplan. Ein Glas jetzt verteilt die Restmenge besser über den Tag.`,`Zum Wasser`);
    const proteinPace=Math.max(25,settings.protein*progress);
    if(total.calories>0&&total.protein<proteinPace-15&&calorieRemaining>120)add(90,'meal','Eiweiß gezielt erhöhen',`Bis zum aktuellen Tageskurs fehlen etwa ${fmt(proteinPace-total.protein)} g Eiweiß; bis zum Tagesziel insgesamt ${fmt(Math.max(0,proteinRemaining))} g.`,`Eiweiß einplanen`,recommendedMealType());
    ...
    else if(settings.steps>0&&stepGap>0&&(!isToday()||hour>=15))add(72,'steps','Bewegungslücke schließen',`${fmt(stepGap)} Schritte fehlen noch bis zum Tagesziel. Eine kurze Runde ist leichter umzusetzen als ein großer Block am Abend.`,`Schritte ansehen`);
  ...
  function renderCoach(total,data,settings,water,score){
    ...
    $('#journalCoachText').textContent=coverage===6?'Alle Kernbereiche sind erfasst. Der Coach priorisiert jetzt Qualität und Zielgenauigkeit.':`${coverage} von 6 Kernbereichen sind belastbar erfasst. Die Empfehlung richtet sich nach Tageszeit, Zielen und deinem aktuellen Stand.`;
    $('#coachV71FocusTitle').textContent=action.title;$('#coachV71FocusReason').textContent=action.reason;
    const button=$('#coachV71Action');button.textContent=action.label;button.dataset.action=action.type;button.dataset.mealType=action.mealType||'';
  ...
  function renderEffects(){
    ...
    animateNumber($('#journalRemaining'),Math.abs(remaining));const eaten=$('#journalEaten');if(eaten)animateNumber(eaten,total.calories,{suffix:' kcal'});if(score!==null){animateNumber($('#journalScoreLarge'),score,{digits:1});animateNumber($('#journalScore'),score,{digits:1})}
