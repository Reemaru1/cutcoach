...
(async()=>{
 ...
 assert.equal(api.version,'1.9.0-alpha');assert.equal(api.build,'1.9.0-search-integrity');
 const dinner=api.rowsFor('250g Steak mit Eier');
 assert.equal(dinner.length,2,'Steak und Eier werden nicht getrennt verarbeitet.');assert.equal(dinner[0].status,'ambiguous','Mehrere passende Steakvarianten müssen auswählbar sein.');assert.ok(dinner[0].choices.some(choice=>/Steak/i.test(choice.item.name)),'Steak-Auswahl fehlt.');assert.equal(dinner[1].item.name,'Ei');
 const potatoes=api.rowsFor('200 g Kartoffeln und 100 g Lachs');assert.equal(potatoes.length,2);assert.equal(potatoes[0].item.id,'bls-kartoffel','Plural Kartoffeln wird nicht generisch aufgelöst.');assert.equal(potatoes[0].factor,2);assert.equal(potatoes[1].item.id,'bls-lachs','Zusammengesetzter Katalogname Lachsfilet wird nicht gefunden.');
 assert.equal(api.rowsFor('1 Tantuni')[0].item.name,'Tantuni Dürüm','Tantuni fehlt als lokales Standardgericht.');
 const input=w.document.querySelector('#nutritionSearch');input.value='tantuni';input.dispatchEvent(new w.Event('input',{bubbles:true}));await wait(240);assert.match(w.document.querySelector('#nutritionMultiSearch').textContent,/Tantuni Dürüm/,'Tantuni wird beim normalen Tippen nicht vorgeschlagen.');
 input.value='250g Steak mit Eier';input.dispatchEvent(new w.Event('input',{bubbles:true}));await wait(240);const host=w.document.querySelector('#nutritionMultiSearch');assert.ok(host.querySelectorAll('[data-confidence-choice]').length>=2,'Steakvarianten sind nicht konkret auswählbar.');
 assert.match(manifest,/nutrition-portion-profiles-v153\.js\?v=1\.5\.3-alpha/);assert.match(manifest,/nutrition-portion-hardening-v153\.js\?v=1\.9\.0-alpha/);assert.match(manifest,/nutrition-search-learning-v161\.js\?v=1\.6\.1-alpha/);assert.match(manifest,/nutrition-search-confidence-hardening-v151\.js\?v=1\.9\.0-alpha/);assert.match(sw,/search153-portions/);assert.match(sw,/search161-hardening/);assert.match(sw,/search190-integrity/);
 dom.window.close();console.log('Generischer Vollkatalog-Resolver und Ranking-Hardening bleiben unter A–Z-Integrität 1.9.0 stabil.');
