...
const {window}=dom;
window.CutCoachNutritionMath206=Object.freeze({version:'test-stub'});
...
assert.ok(smart.includes("journal-live-nav-v127.js?v=1.3.3-alpha"),'Smart-Journal lädt nicht die reparierte Live-Navigation.');
assert.doesNotMatch(live,/lastChild\.textContent='Suchen'/,'Der Lebensmittel-Shortcut wird weiterhin irreführend als Suche umbenannt.');
...
assert.ok(runtime.includes("journal-live-nav-v127.js?v=1.3.3-alpha"),'Runtime-Manifest enthält nicht die reparierte Live-Navigation.');
...
assert.ok(sw.includes('-nav136-journal137-nutrition138-dishes140-dashboard820`'),'Service Worker verwendet nicht die aktuelle Navigation-, Tagebuch-, Ernährungs- und Dashboard-Cachegeneration.');
assert.ok(sw.includes('search196-article-sequence'),'Navigationstest sieht die aktuelle Such-Cachegeneration nicht.');

dom.window.close();
console.log('Flackerfreies Instagram-Dock mit unverändertem grünem Plusbutton unter Runtime 2.3.0 geprüft.');
