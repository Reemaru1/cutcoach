...
function rowTargets(row){const ids=[];if(row?.item?.id!==undefined)ids.push(String(row.item.id));for(const choice of row?.choices||[])if(choice?.item?.id!==undefined)ids.push(String(choice.item.id));return ids}
function containsTarget(rows,item){const target=String(item.id);return rows.some(row=>rowTargets(row).includes(target))}
function inspectName(item,group,name=item.name){const query=queryForName(item,name),rows=api.rowsFor(query);return containsTarget(rows,item)?null:{group,id:String(item.id),name:item.name,testedName:name,query,statuses:rows.map(row=>row.status),resolved:rows.map(row=>({name:row.item?.name||null,id:row.item?.id||null,choices:(row.choices||[]).map(choice=>choice.item?.id||null)}))}}

const auditItems=[],seenEntries=new Set();
...
const aliasChecks=[];
for(const {group,item} of auditItems)for(const alias of Array.isArray(item.aliases)?item.aliases:[item.aliases])if(alias&&normalize(alias)!==normalize(item.name))aliasChecks.push({group,item,alias});
...
for(const [name,items] of exactGroups){const unique=[...new Map(items.map(item=>[String(item.id),item])).values()];if(unique.length<2)continue;const rows=api.rowsFor(queryForName(unique[0])),first=rows[0];if(first?.status==='matched'&&!first?.choices?.length)unsafeDuplicateSelections.push({name,ids:unique.slice(0,6).map(item=>item.id),selected:first.item?.id||null})}
...
for(const query of nonsense){const matched=api.rowsFor(query).filter(row=>row.status==='matched'&&row.item);if(matched.length)falseUnknownMatches.push({query,matches:matched.map(row=>({id:row.item.id,name:row.item.name,confidence:row.confidence}))})}
const protectedDishFailures=[];
for(const item of [...localItems,...everydayItems].filter(item=>/\b(mit|und|auf|ohne)\b|\//i.test(item.name))){const rows=api.rowsFor(queryForName(item));if(!containsTarget(rows,item))protectedDishFailures.push({id:item.id,name:item.name,query:queryForName(item),rows:rows.map(row=>({status:row.status,id:row.item?.id||null,name:row.item?.name||null}))})}
...
assert.ok(indexStats.origins.catalog>=foodItems.length,'Der Katalogursprung fehlt im Stufe-5-Index.');
assert.ok(indexStats.origins.everyday>=everydayItems.length,'Der Alltagsursprung fehlt im Stufe-5-Index.');
...
console.log(`Stufe 5 Vollkatalog-Audit 1.7.1: ${auditItems.length} vollständige Einträge, ${aliasChecks.length} Aliase, ${exactGroups.size} Namensgruppen und ${nonsense.length} Negativfälle in ${durationMs} ms geprüft.`);
dom.window.close();