...
row=api.rowsFor('1 EL Whey')[0];assert.equal(row.status,'matched');assert.ok(Math.abs(row.factor-.4)<.0001);assert.equal(row.confidence,98);assert.equal(row.portionSource,'item');
