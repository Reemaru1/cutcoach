...
source=source.replaceAll("'Version 7.1.0'","'Version 7.3.0'");
source=source.replaceAll("'7.1.0','Zentrale Releasekonstante fehlt'","'7.3.0','Zentrale Releasekonstante fehlt'");
source=source.replace(/version:'7\\\.1\\\.0'/g,"version:'7\\.3\\.0'");
source=source.replace(/runtime-manifest\\\.js\\\?v=7\\\.1\\\.0/g,'runtime-manifest\\.js\\?v=7\\.3\\.0');
source=source.replace(/sw\\\.js\\\?v=7\\\.1\\\.0-force/g,'sw\\.js\\?v=7\\.3\\.0-force');
source=source.replaceAll('updated=710','updated=730');
source=source.replace("  console.log('CutCoach 7.1 smoke test: ok');","  assert.ok(window.CutCoachEverydayCatalog,'Alltagskatalog fehlt im vollständigen App-Start');\n  assert.ok(window.CutCoachEverydayCatalog.meta.count>=55,'Zu wenige Alltagsgerichte im App-Start');\n  for(const asset of ['journal-v72.js','journal-v72.css','everyday-catalog-v73.js','nutrition-v73.js','nutrition-v73.css']){\n    assert.ok(manifest.includes(`./${asset}?v=${asset.startsWith('journal-v72')?'7.2.0':'7.3.0'}`),`Offline-Manifest enthält ${asset} nicht`);\n    assert.ok(fs.existsSync(path.join(project,asset)),`Datei fehlt: ${asset}`);\n  }\n  console.log('CutCoach 7.3 compatibility smoke test: ok');");
