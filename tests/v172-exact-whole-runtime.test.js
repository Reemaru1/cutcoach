...
assert.equal(api.exactWholeVersion,'1.9.0-alpha');assert.equal(api.exactWholeBuild,'1.9.0-search-integrity');assert.equal(api.version,'1.9.0-alpha');assert.equal(api.build,'1.9.0-search-integrity');
...
assert.match(compatibility,/const VERSION='1\.9\.0-compat'/);assert.match(compatibility,/nutrition-search-exact-whole-v170\.js\?v=1\.9\.0-alpha/);assert.match(compatibility,/nutrition-portion-hardening-v153\.js\?v=1\.9\.0-alpha/);
...
assert.match(sw,/search170-exact-whole/);assert.match(sw,/search171-edge-hardening/);assert.match(sw,/stage6-production180/);assert.match(sw,/search190-integrity/);
inject(w,compatibility);assert.equal(w.CutCoachNutritionMultiSearch120.version,'1.9.0-compat');assert.equal(w.CutCoachNutritionMultiSearch120.resolve('550 g Lachs mit Reis und Gemüse').match.id,dish.id);assert.equal(w.CutCoachNutritionMultiSearch120.resolve('0 g Menemen').invalidQuantity,true);const multi=w.CutCoachNutritionMultiSearch120.resolve('Lachs mit Reis und Gemüse + Cola');assert.equal(multi.match,null);assert.equal(multi.multiple,true);assert.equal(multi.status,'multiple');
dom.window.close();console.log('Exact-Whole unter A–Z-Suchintegrität 1.9.0: Vollnamen, Kombinationen, natürliche Sprache, Zahlenamen, Nullmengen, Kollisionen, Einheiten, Index und Ladefolge geprüft.');
