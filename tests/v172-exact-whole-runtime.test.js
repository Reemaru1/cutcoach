...
const catalog=[dish,goose,menemen,puddingOne,puddingTwo,numeric,cola];
...
const w=dom.window;
w.CutCoachLibrary={exportData:()=>({items:[]}),addItemToDay:()=>({id:'meal'}),addCatalogItemToDay:()=>({id:'meal'}),undoDayAdd:()=>true};w.CutCoachFoodCatalog={items:()=>catalog,get:id=>catalog.find(item=>item.id===id)||null};w.CutCoachEverydayCatalog={items:()=>[],get:()=>null};w.render=()=>{};w.toast=()=>{};
...
let engine=w.CutCoachIntelligentSearch128;engine=w.CutCoachSearchExactWhole170.attach(engine);engine=w.CutCoachSearchConfidenceHardening151.attach(engine);engine=w.CutCoachPortionHardening153.attach(engine);
...
let rows=api.rowsFor('550 g Lachs mit Reis und Gemüse');assert.equal(rows.length,1);assert.equal(rows[0].item.id,dish.id);assert.equal(rows[0].status,'matched');assert.equal(rows[0].factor,1);assert.notEqual(rows[0].matchType,'fuzzy');
...
rows=api.rowsFor('550 g Lachs mit Reis und Gemüse und 330 ml Cola');assert.equal(rows.length,2);assert.equal(rows[0].item.id,dish.id);assert.equal(rows[1].item.id,cola.id);
rows=api.rowsFor('Lachs mit Reis und Gemüse + Cola');assert.equal(rows.length,2);assert.equal(rows[0].item.id,dish.id);assert.equal(rows[1].item.id,cola.id);
...
api.rowsFor('100 g Neue Schutzspeise');const newItem={id:'new-protected',name:'Neue Schutzspeise mit Reis',amount:100,unit:'g',calories:200,protein:10,carbs:25,fat:5,source:'user'};catalog.push(newItem);assert.equal(api.rowsFor('100 g Neue Schutzspeise mit Reis').some(row=>row.item?.id===newItem.id),false,'Index wurde ohne Ereignis unerwartet neu aufgebaut.');w.dispatchEvent(new w.CustomEvent('cutcoach:catalog-updated'));rows=api.rowsFor('100 g Neue Schutzspeise mit Reis');assert.equal(rows.length,1);assert.equal(rows[0].item.id,newItem.id);
...
assert.match(loader,/nutrition-search-exact-whole-v170\.js\?v=1\.9\.6-alpha/);assert.match(loader,/nutrition-search-confidence-hardening-v151\.js\?v=1\.9\.0-alpha/);assert.match(loader,/nutrition-portion-hardening-v153\.js\?v=1\.9\.0-alpha/);assert.match(loader,/CutCoachSearchExactWhole170\?\.attach[\s\S]*CutCoachSearchConfidenceHardening151\?\.attach[\s\S]*CutCoachPortionHardening153\?\.attach/);
assert.match(compatibility,/const VERSION='1\.9\.6-compat'/);assert.match(compatibility,/nutrition-search-exact-whole-v170\.js\?v=1\.9\.6-alpha/);assert.match(compatibility,/nutrition-portion-hardening-v153\.js\?v=1\.9\.0-alpha/);
...
assert.match(sw,/search170-exact-whole/);assert.match(sw,/search171-edge-hardening/);assert.match(sw,/stage6-production180/);assert.match(sw,/search190-integrity/);assert.match(sw,/search196-article-sequence/);
inject(w,compatibility);assert.equal(w.CutCoachNutritionMultiSearch120.version,'1.9.6-compat');assert.equal(w.CutCoachNutritionMultiSearch120.resolve('550 g Lachs mit Reis und Gemüse').match.id,dish.id);assert.equal(w.CutCoachNutritionMultiSearch120.resolve('0 g Menemen').invalidQuantity,true);const multi=w.CutCoachNutritionMultiSearch120.resolve('Lachs mit Reis und Gemüse + Cola');assert.equal(multi.match,null);assert.equal(multi.multiple,true);assert.equal(multi.status,'multiple');
dom.window.close();console.log('Exact-Whole 1.9.6: Vollnamen, Artikel, Sequenzen, Kombinationen, natürliche Sprache, Zahlenamen, Nullmengen, Kollisionen, Einheiten, Index und Ladefolge geprüft.');