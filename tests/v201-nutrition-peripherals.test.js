...
(async()=>{
  {
    ...
    const w=dom.window;inject(w,'nutrition-cleanup-101.js');await wait(30);
    ...
    const snapshot=w.CutCoachNutritionCleanup101.snapshot();assert.equal(snapshot.scoped,true);assert.ok(snapshot.syncCount<8,'Cleanup läuft in einer Mutationsschleife.');
  ...
  }
  {
    ...
    const w=dom.window;inject(w,'nutrition-voice-111.js');await wait(10);const api=w.CutCoachNutritionVoice111;assert.equal(api.version,'1.9.1-alpha');api.clearStaleResults();
  ...
  console.log('Ernährungsperipherie 2.0.1: Cleanup, Sprache, Open Food Facts und Scanner stabil.');
