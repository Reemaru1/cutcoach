...
(async()=>{
  ...
  assert.ok(sw.includes('`${EDGE_CACHE}-energy143`'),'Die Cachegeneration für Tageskurs und Stufe-5-Randfallhärtung ist nicht verkettet.');
  assert.ok(sw.includes('search170-exact-whole'),'Stufe-5-Vollnamen-Cachegeneration fehlt.');
  assert.ok(sw.includes('search171-edge-hardening'),'Stufe-5-Randfall-Cachegeneration fehlt vor dem Tageskurs-Cache.');
