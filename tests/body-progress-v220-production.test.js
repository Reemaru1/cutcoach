...
(async()=>{
  ...
  assert.match(read('body-progress-v220.css'),/html \.bp220-modal \.two\{grid-template-columns:repeat\(2,minmax\(0,1fr\)\)!important\}/,'Die Formularbreiten hängen weiterhin fälschlich vom aktiven Progress-Screen ab.');
  assert.match(read('body-progress-v220.css'),/\.bp220-workout-meta\{grid-template-columns:1fr!important\}/,'Datum und Dauer werden auf schmalen iPhones weiterhin nebeneinander gequetscht.');
  assert.match(read('body-progress-v220.css'),/html \.bp220-modal #bp220SaveWorkout\{[^}]*background:linear-gradient\(145deg,rgba\(255,112,88,.12\)/,'Die Trainingsaktion nutzt nicht die neue ruhige Premium-Sprache.');
  assert.ok(w.document.querySelector('[data-bp220-remove-exercise] svg'),'Der alte X-Button wurde nicht durch eine eindeutige Löschen-Aktion ersetzt.');
  ...
  assert.match(read('index.html'),/body-progress-v220\.js\?v=2\.2\.4-production/);
