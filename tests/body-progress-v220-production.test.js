...
(async()=>{
  ...
  for(const file of ['core.js','body-progress-v220.js']){const script=w.document.createElement('script');script.textContent=read(file);w.document.head.append(script)}
  ...
  assert.equal(w.document.querySelectorAll('nav [data-bp220-training-nav]').length,1,'Die Fortschrittsnavigation braucht den Trainingsmodus.');
  ...
  assert.doesNotMatch(read('body-progress-v220.js'),/Fettverbrennung im Bauchbereich|92%|18,6\s*t|7,8\s*\/10/,'Die Produktion darf keine erfundenen Referenzmetriken enthalten.');
  ...
  assert.match(read('index.html'),/body-progress-v220\.js\?v=2\.2\.0-production/);
  assert.match(read('runtime-manifest.js'),/assets\/body-progress-training-v3\.png\?v=2\.2\.0/);
  assert.match(read('sw.js'),/body220-production/);
  dom.window.close();
  console.log('Body Progress 2.2.0: echte Körpermessungen, Trainingsvolumen, Muskelmapping, Regeneration, Produktion und Offline-Assets geprüft.');
