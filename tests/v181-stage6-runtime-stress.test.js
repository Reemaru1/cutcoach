...
(async()=>{
  ...
  const w=dom.window,input=w.document.querySelector('#nutritionSearch'),host=w.document.querySelector('#nutritionMultiSearch');inject(w,stage6);const api=w.CutCoachNutritionStage6;assert.ok(Object.isFrozen(api));
  ...
  input.value='';input.dispatchEvent(new w.Event('input',{bubbles:true}));assert.equal(host.innerHTML,'');assert.equal(w.document.body.classList.contains('canonical-multisearch-active'),false);
  dom.window.close();
