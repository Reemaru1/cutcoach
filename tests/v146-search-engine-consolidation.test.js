...
assert.match(portions,/kind:'choice'/);
...
assert.match(exactWhole,/const VERSION='1\.9\.0-alpha'/);
assert.match(exactWhole,/search-integrity/);
assert.match(exactWhole,/protectedRows/);
assert.match(exactWhole,/protectedSequenceRows/);
assert.match(exactWhole,/invalidQuantity/);
assert.match(hardening,/const VERSION='1\.9\.0-alpha'/);
assert.match(hardening,/search-integrity/);
...
const facade=window.CutCoachNutritionMultiSearch120;engine=window.CutCoachIntelligentSearch128;assert.ok(engine);assert.ok(facade);assert.equal(engine.version,'1.9.0-alpha');assert.equal(engine.build,'1.9.0-search-integrity');assert.equal(engine.learningVersion,'1.6.1-alpha');assert.equal(engine.exactWholeVersion,'1.9.0-alpha');assert.equal(facade.mode,'compatibility-facade');assert.equal(facade.engineVersion(),engine.version);
...
assert.match(loader,/const loadConfidence=\(\)=>ensure\('nutrition-search-confidence-hardening-v151','\.\/nutrition-search-confidence-hardening-v151\.js\?v=1\.9\.0-alpha'/);
assert.match(loader,/const loadExactWhole=\(\)=>ensure\('nutrition-search-exact-whole-v170','\.\/nutrition-search-exact-whole-v170\.js\?v=1\.9\.0-alpha'/);
...
assert.match(manifest,/nutrition-portion-profiles-v153\.js\?v=1\.5\.3-alpha/);assert.match(manifest,/nutrition-portion-hardening-v153\.js\?v=1\.9\.0-alpha/);assert.match(manifest,/nutrition-search-learning-v161\.js\?v=1\.6\.1-alpha/);assert.match(manifest,/nutrition-search-exact-whole-v170\.js\?v=1\.9\.0-alpha/);assert.match(manifest,/nutrition-search-confidence-hardening-v151\.js\?v=1\.9\.0-alpha/);assert.match(manifest,/nutrition-multisearch-120\.js\?v=1\.9\.0-compat/);assert.match(sw,/search153-portions/);assert.match(sw,/search160-learning/);assert.match(sw,/search161-hardening/);assert.match(sw,/search170-exact-whole/);assert.match(sw,/search171-edge-hardening/);assert.match(sw,/stage6-production180/);assert.match(sw,/search190-integrity/);
dom.window.close();console.log('Eine zentrale Such-Engine mit Vollkatalog-, Vollnamen-, Portions-, Lern- und A–Z-Integrität: ok');
