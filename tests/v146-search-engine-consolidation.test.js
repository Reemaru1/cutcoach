...
assert.match(compatibility,/const VERSION='1\.5\.2-compat'/);
...
const engine=window.CutCoachIntelligentSearch128,facade=window.CutCoachNutritionMultiSearch120;assert.ok(engine);assert.ok(facade);assert.equal(engine.version,'1.5.2-alpha');assert.equal(facade.mode,'compatibility-facade');assert.equal(facade.engineVersion(),engine.version);
...
assert.match(manifest,/nutrition-search-confidence-hardening-v151\.js\?v=1\.5\.2-alpha/);assert.match(manifest,/nutrition-multisearch-120\.js\?v=1\.5\.0-compat/);assert.match(sw,/search152-catalog/);
dom.window.close();console.log('Eine zentrale Such-Engine mit generischem Vollkatalog-Resolver und passiver Brücke: ok');