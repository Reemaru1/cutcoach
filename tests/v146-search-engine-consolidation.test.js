...
assert.match(compatibility,/nutrition-search-learning-v160/);
...
assert.match(learning,/const VERSION='1\.6\.0-alpha'/);
assert.match(learning,/local-learning/);
assert.match(learning,/MAX_AGE_MS=180/);
assert.doesNotMatch(learning,/document\.addEventListener/);
...
const facadeScript=window.document.createElement('script');facadeScript.textContent=compatibility;window.document.head.append(facadeScript);assert.equal(listenerCount,activeEngineListeners,'Lernschicht oder passive Kompatibilitätsbrücke registriert zusätzliche Dokument-Listener.');
const facade=window.CutCoachNutritionMultiSearch120;engine=window.CutCoachIntelligentSearch128;assert.ok(engine);assert.ok(facade);assert.equal(engine.version,'1.5.3-alpha');assert.equal(engine.build,'1.5.3-food-specific-portions');assert.equal(engine.learningVersion,'1.6.0-alpha');assert.equal(facade.mode,'compatibility-facade');assert.equal(facade.engineVersion(),engine.version);
...
assert.match(loader,/nutrition-multisearch-120\.js\?v=1\.5\.3-compat/);assert.doesNotMatch(loader,/loadLegacy/);
...
dom.window.close();console.log('Eine zentrale Such-Engine mit Vollkatalog-, Portions- und lokaler Lernschicht: ok');