...
window.CutCoachEverydayCatalog={items:()=>[],get:byId};
...
assert.ok(loader.includes('nutrition-multisearch-canonical-128.js?v=1.5.0-alpha'));assert.ok(runtime.includes('nutrition-multisearch-canonical-128.js?v=1.5.0-alpha'));assert.ok(sw.includes('`${CACHE_BASE}-search150-confidence`'));
dom.window.close();console.log('Sprache, Einheiten, Komposita und Teiltreffer mit Confidence Score 1.5.0 geprüft.');