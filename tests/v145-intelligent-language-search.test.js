...
assert.ok(loader.includes('nutrition-portion-profiles-v153.js?v=1.5.3-alpha'));assert.ok(loader.includes('nutrition-portion-hardening-v153.js?v=1.9.0-alpha'));assert.ok(loader.includes('nutrition-multisearch-canonical-128.js?v=1.5.0-alpha'));assert.ok(runtime.includes('nutrition-portion-hardening-v153.js?v=1.9.0-alpha'));assert.ok(runtime.includes('nutrition-multisearch-canonical-128.js?v=1.5.0-alpha'));assert.ok(sw.includes('`${CACHE_BASE}-search153-portions`'));assert.ok(sw.includes('search190-integrity'));
dom.window.close();console.log('Sprache, Einheiten, Komposita und Teiltreffer bleiben unter A–Z-Suchintegrität 1.9.0 stabil.');
