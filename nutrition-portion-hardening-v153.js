...
(function(global){
  ...
  let base=null,api=null,timer=0,renderToken=0,rendering=false,composing=false;

  function confidenceLabel(value){const score=Math.round(clamp(value,0,100));if(score>=98)return`Exakt · ${score}%`;if(score>=90)return`Sehr sicher · ${score}%`;if(score>=72)return`Bitte prüfen · ${score}%`;return`Menge prüfen · ${score}%`}
  ...
  function rowsFor(value){return base?applyRows(base.rowsFor?.(value)||[]):[]}
  function hasHouseholdInput(value){return rowsFor(value).some(isHouseholdRow)}
  ...
  function renderRows(input,rows){
    const node=host();if(!node||!rows.length)return false;rendering=true;
    ...
    node.dataset.canonical='1';node.dataset.query=normalize(input.value);node.dataset.portion153='1';node.hidden=false;node._canonicalRows=rows;suppressNormalResults(true);
  ...
  function render(input){if(!base||!input)return false;const rows=rowsFor(input.value);if(!rows.some(isHouseholdRow))return Boolean(base.render?.(input));return renderRows(input,rows)}
  ...
  function selectChoice(button){
    ...
    const [rowIndex,choiceIndex]=String(button.dataset.confidenceChoice||'').split(':').map(Number),rows=[...node._canonicalRows],row=rows[rowIndex],choice=row?.choices?.[choiceIndex];if(!choice)return false;
  ...
  document.addEventListener('compositionend',event=>{if(event.target?.id!=='nutritionSearch'||!api)return;composing=false;if(!hasHouseholdInput(event.target.value))return;event.preventDefault();event.stopImmediatePropagation();schedule(event.target)},true);
  document.addEventListener('input',event=>{const input=event.target;if(input?.id!=='nutritionSearch'||!api||composing||input.dataset.composing==='1'||input.dataset.voicePreview==='1'||!hasHouseholdInput(input.value))return;event.preventDefault();event.stopImmediatePropagation();schedule(input)},true);
  document.addEventListener('keydown',event=>{const input=event.target;if(input?.id!=='nutritionSearch'||!api||event.key!=='Enter'||!hasHouseholdInput(input.value))return;event.preventDefault();event.stopImmediatePropagation();clearTimeout(timer);render(input)},true);
  ...
  global.CutCoachPortionHardening153=Object.freeze({version:VERSION,build:BUILD,attach,applyRows});
})(window);