...
(function(){
  ...
  function waterFor(key=selectedDate){return clamp(Number(readWater()[key]||0),0,10000)}
  function setWater(value,key=selectedDate){const all=readWater(),amount=Math.round(clamp(Number(value)||0,0,10000));if(amount)all[key]=amount;else delete all[key];try{localStorage.setItem(WATER_KEY,JSON.stringify(all))}catch{}window.render()}
  ...
  window.completionStatus=function(){
    const t=totals(),data=day(selectedDate,false),items=[['Ernährung',t.calories>0],['Schritte',data.steps!==null||state.settings.steps===0],['Wasser',waterFor()>0],['Training',data.gym!==null],['Alkohol',data.alcohol!==null]];
  ...
  window.dailyScore=function(){
    ...
    const waterScore=clamp(waterFor()/WATER_TARGET,0,1)*.5;
    const tracking=[data.steps!==null||settings.steps===0,data.gym!==null,data.alcohol!==null,waterFor()>0].filter(Boolean).length/4*.25;
  ...
  function dayStatus(t,settings,effective){
    ...
    if(waterFor()<1500)return {text:'💧 Trinken nicht vergessen',tone:'focus'};
  ...
  function focusLine(t,settings,effective){
    const proteinGap=Math.max(0,settings.protein-t.protein),remaining=settings.calories-t.calories,band=fatBand(settings),water=waterFor();
    ...
    if(water<WATER_TARGET*.6)return `🎯 Noch ${fmt(WATER_TARGET-water)} ml Wasser über den Tag verteilen`;
  ...
  window.feedback=function(){
    ...
    if(waterFor()>=WATER_TARGET)lines.push('✅ Hydration: Tagesziel erreicht');else if(data.alcohol===true)lines.push('⚠️ Regeneration durch Alkohol reduziert');else if(data.alcohol===false)lines.push('✅ Regeneration: alkoholfrei');
  ...
  function ensureWaterCard(){
    ...
    steps.insertAdjacentHTML('afterend','<section class="card water-card" id="waterCard"><label><span class="card-title">💧 Wasser</span><span class="card-meta">Ziel 3,0 l</span></label><div class="water-layout"><div class="water-orb" aria-hidden="true"><div class="water-fill" id="waterFill"></div><div class="water-value"><strong id="waterAmount">0</strong><small>ml</small></div></div><div class="water-controls"><div class="water-quick"><button type="button" data-water-add="250">+250 ml</button><button type="button" data-water-add="500">+500 ml</button></div><button type="button" class="water-undo" id="waterUndo">−250 ml</button><p id="waterHint">Starte mit dem ersten Glas.</p></div></div><div class="water-drops" id="waterDrops" aria-label="Wasserfortschritt"></div></section>');
    const card=document.querySelector('#waterCard');addInfoButton(card,'Wasser','Das Tagesziel liegt bei 3 Litern. Wasser unterstützt Leistungsfähigkeit, Sättigung und Regeneration. Kaffee und ungesüßter Tee können mitgezählt werden.');
  ...
  function renderWater(){
    const amount=waterFor(),ratio=clamp(amount/WATER_TARGET,0,1),fill=document.querySelector('#waterFill');
    setText('#waterAmount',fmt(amount));if(fill)fill.style.height=`${Math.max(amount?8:0,ratio*100)}%`;
    const hint=amount===0?'Starte mit dem ersten Glas.':amount>=WATER_TARGET?'Tagesziel erreicht – stark!':`${fmt(WATER_TARGET-amount)} ml bis zum Tagesziel`;
    setText('#waterHint',hint);const undo=document.querySelector('#waterUndo');if(undo)undo.disabled=amount===0;
    const drops=document.querySelector('#waterDrops');if(drops){drops.replaceChildren();for(let i=1;i<=6;i++){const dot=document.createElement('span');dot.className=i<=Math.ceil(ratio*6)?'filled':'';dot.textContent='💧';drops.append(dot);}}
    document.querySelector('#waterCard')?.classList.toggle('goal-reached',amount>=WATER_TARGET);
  }
  function ensureUi(){
