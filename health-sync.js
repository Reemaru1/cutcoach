'use strict';
(function(){
  const VERSION='5.2.0',META='cutcoach_health_sync_v1',PAIR='cutcoach_cloud_pair_v1',MAX=100000;
  ...
  function remember(payload,previous){const meta=read(META,{}),syncedAt=new Date().toISOString(),entry={date:payload.date,steps:payload.steps,previous,syncedAt,source:payload.source,context:standalone()?'standalone':'browser',cloudUpdatedAt:payload.cloudUpdatedAt||null};meta.version=3;meta.last=entry;meta.days=meta.days&&typeof meta.days==='object'?meta.days:{};meta.days[payload.date]=entry;write(META,meta)}
  function apply(payload,{silent=false}={}){const data=day(payload.date,true),previous=data.steps;data.steps=payload.steps;if(!saveState(true)){data.steps=previous;return false}remember(payload,previous);window.render();if(!silent)toast(`Health-Cloud: ${fmt(payload.steps)} Schritte übernommen`);return true}
  async function pull({silent=true,force=false}={}){if(!validToken(token(false))||!navigator.onLine)return null;if(!force&&Date.now()-lastPull<30000)return null;if(pendingPull)return pendingPull;lastPull=Date.now();pendingPull=(async()=>{try{const payload=await fetchDay();if(!payload)return null;const current=day(payload.date,false).steps;if(current!==payload.steps)apply(payload,{silent});return payload}catch(error){patchPair({cloudReady:false,lastError:String(error?.message||error)});if(!silent)toast('Cloud nicht erreichbar.');window.render();return null}finally{pendingPull=null}})();return pendingPull}
  ...
  window.CutCoachHealthSync={version:VERSION,pull,push,testCloud,ensureToken:()=>token(true),getToken:()=>token(false),setToken,shortcutSuffix:suffix,personalShortcutTemplate:template,pairData:pair,copyText:copy};
  window.render=function(){baseRender();post()};
  window.addEventListener('pageshow',()=>pull({silent:true}));
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)pull({silent:true})});
  setInterval(()=>{if(!document.hidden)pull({silent:true})},300000);
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{window.render();setTimeout(()=>pull({silent:true}),80)},{once:true});else{window.render();setTimeout(()=>pull({silent:true}),80)}
})();