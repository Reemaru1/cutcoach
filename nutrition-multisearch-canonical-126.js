...
(function(){
  ...
  function keyFor(query){
    ...
    if(q==='doner'||q==='doener'||q==='döner'||q==='kebab'||q==='kebap')return'doener';
    if(q==='durum'||q==='dueruem'||q==='dürüm'||q==='yufka')return'dueruem';
    if(q==='butterbreze'||q==='butterbrezel'||q==='butterbrezn')return'butterbreze';
    if(q==='kasesemmel'||q==='kaesesemmel'||q==='käse semmel'||q==='kasebrotchen'||q==='käsebrötchen')return'kaesesemmel';
    return null;
  }
  ...
  function suppressNormalResults(active){
    ...
    const title=document.querySelector('.nutrition-results-title, #nutritionResultsTitle');
    if(active&&title)title.hidden=true;
  }
  ...
  document.addEventListener('input',event=>{
    ...
    event.stopImmediatePropagation();clearTimeout(timer);timer=setTimeout(()=>render(event.target),160);
  },true);
